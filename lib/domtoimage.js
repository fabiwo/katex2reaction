;(function (global) {
  ...
  var domtoimage = {
    toStyledNodeClone: toStyledNodeClone,
  ...
  ///// OWN IMPLEMENTATION ///////////////////
  function toStyledNodeClone(node, options) {
    options = options || {}
    copyOptions(options)
    return Promise.resolve(node)
      .then(function (node) {
        return cloneNodeWithoutStyle(node, options.filter, true)
      })
      .then(embedFonts)
      .then(inlineImages)
      .then(function (clone) {
  ...
  function cloneNodeWithoutStyle(node, filter, root) {
    ...
    return Promise.resolve(node)
      ...
      .then(function (clone) {
        return processCloneNoStyle(node, clone)
      })
    ...
    function processCloneNoStyle(original, clone) {
  ...
  function toSvgString(node, options) {
    return toStyledNodeClone(node, options).then((clone) => {
      const width = util.width(node)
      const height = util.height(node)

      return makeSvg(clone, width, height)
    })
  }

  function makeSvg(node, width, height) {
    return Promise.resolve(node)
      .then(function (node) {
